...
- 詳細画面に、リポジトリの詳細をWebで開くボタンを追加した。
- 詳細画面に、リポジトリのREADME.mdを表示する機能を追加した。
- MVVMアーキテクチャを採用し、ViewControllerの責務を明確にした。
- APIクライアントを実装し、MainVCから通信処理を切り離した。
- 検索時にローディングアニメーションを表示するようにした。
- 通信に失敗した場合にアラートを表示するようにした。
